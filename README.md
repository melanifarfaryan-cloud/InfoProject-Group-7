...
## VERSION 4:
This is the version with more features, funtions and improvements. It is out most important version. It contains the fundamental required funtions of version 4 plus our spectacular funcions that will suprise the user, including a highly devolped improvemnt of the interface.
In this version, we introduced the management of both aircraft arrivals and departures at Barcelona airport (LEBL), making the gate assignment a dynamic process throughout the day. We updated the Aircraft structure (destination_airport and time_of_departure). We implemented functionalities to load departures from a file (LoadDepartures) and mix them with arrivals based on compatible times (MergeMovements), adding also aircraft that spent the night at the airport (NightAircraft). The gate assignment process is now dynamic: the tool updates the state of the airport hour by hour (AssignGatesAtTime), freeing gates as aircraft depart so they can be reused by flights landing later in the day. Additionally, we implemented a global occupancy analysis function (PlotDayOccupancy) to show the total number of gates assigned per terminal and the number of unassigned flights due to full occupancy in every hour period. We added clarifying comments using hashtags (#) inside the code, and extended the interface so users can carry out all functionalities from previous versions along with the new features. We have added a standby screen with a game and backround music. Our  extra funcionalities are funtions to simulate delays, a dynamic googlle earth map, a delay simulator, a meteo simulator, our best extra funcionality is a google earth map with a time bar that you mov it manually and it shows you the flights that have flown in one day after this exact time, and also we have created a mini game for our standby screen. This extra funcions, and the interface improvement will aslso be explained in detail in the video for version 4.

Link video: https://youtu.be/ErJGVI6vvJE?si=P-wwSqQHPVdum8LR
